...
import { DEFAULT_LANDING_ABOUT, LandingAboutContent, LandingAboutMember } from '../../lib/siteContent';
...
export default function AboutValues({ onLearnMoreClick, isSubPage = false, content = DEFAULT_LANDING_ABOUT }: AboutValuesProps) {
  ...
  const fallbackTeamMembers: Array<Omit<LandingAboutMember, 'is_visible'>> = [
    {
      id: 'edwar',
  ...
  const teamMembers = (content.team_members?.length ? content.team_members : fallbackTeamMembers.map(member => ({ ...member, is_visible: true }))).filter(member => member.is_visible !== false);
    ...
    return (
      <section id="sobre-el-iics" className={`${isSubPage ? 'pt-2 sm:pt-4 pb-20 bg-transparent' : 'py-24 bg-black'} border-t border-zinc-950 relative overflow-hidden`}>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-14">
          <div className="text-center md:text-left border-b border-zinc-900 pb-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-stretch border-b border-zinc-900/40 pb-12">
            <div className="lg:col-span-7 space-y-4 text-left">
              ...
              <div className="space-y-4 text-zinc-300 text-sm leading-relaxed">
                <p className="font-sans">{content.story_paragraph_one}</p>
                <p className="font-sans">{content.story_paragraph_two}</p>
                <div className="text-xs text-zinc-300 p-5 bg-white/[0.015] border border-white/[0.06] rounded-none relative overflow-hidden backdrop-blur-sm shadow-inner hover:border-cyan-500/20 transition-all duration-300">
                  <div className="relative z-10 flex gap-3 text-left">
                    <Quote className="h-5 w-5 text-cyan-400 flex-shrink-0 opacity-40" />
                    <p className="italic font-medium leading-relaxed text-zinc-300">"{content.quote}"</p>
                  </div>
                </div>
                <p className="font-sans">{content.story_paragraph_three}</p>
              </div>
            </div>
            ...
            <motion.div
              ...
              className="lg:col-span-5 flex flex-col justify-between relative p-6 sm:p-8 glass-card rounded-none overflow-hidden group min-h-[360px] hover:border-cyan-500/25 transition-all duration-300"
            >
              ...
              <div className="relative z-10 text-left space-y-5">
                <span className="text-[10px] font-mono font-black text-cyan-400 uppercase tracking-widest">IICS / ADN institucional</span>
                <h4 className="text-2xl font-black text-white tracking-tight">{content.network_title}</h4>
                <p className="text-sm text-zinc-300 leading-relaxed">{content.network_intro}</p>
                <div className="p-4 border border-white/[0.06] bg-black/40 text-[11px] text-zinc-400 leading-relaxed">
                  {content.network_support}
                </div>
              </div>
              <div className="relative z-10 mt-8 grid grid-cols-2 gap-3">
                {['Autonomia', 'Rigor', 'Territorio', 'Datos'].map((item, index) => (
                  <div key={item} className="border border-cyan-500/10 bg-cyan-950/[0.03] p-3">
                    <span className="text-[9px] font-mono text-cyan-400 font-black">0{index + 1}</span>
                    <p className="text-xs text-white font-bold uppercase tracking-wider mt-1">{item}</p>
                  </div>
                ))}
              </div>
            </motion.div>
          ...
          <div className="border-b border-zinc-900/40 pb-12">
            <div className="text-left mb-6 flex flex-col md:flex-row md:items-end justify-between gap-6">
              ...
              <div className="flex bg-white/[0.015] backdrop-blur-md border border-white/[0.06] p-1.5 rounded-none self-start md:self-auto shadow-lg">
                {[
                  ...
                  { id: 'academico', label: 'Cuerpo Academico' }
                ].map((tab) => (
                  <button
                    key={tab.id}
                      ...
                      <div className="flex items-center justify-between border-b border-white/[0.06] pb-3 mb-5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[9px] font-mono text-zinc-500 uppercase tracking-widest font-black">
                            {member.category === 'promotor' ? 'FUNDADOR / DIRECTIVO' : 'CUERPO ACADEMICO'}
                          </span>
                        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
            ...
            <motion.div
              whileHover={{ y: -4 }}
              transition={{ duration: 0.3 }}
              className="lg:col-span-6 flex items-center justify-center relative min-h-[385px] border border-white/[0.06] rounded-none overflow-hidden select-none hover:border-cyan-500/25 transition-all duration-300 bg-transparent"
            >
              ...
              <div className="absolute top-4 left-4 flex flex-col font-mono text-[9px] text-zinc-400 tracking-wider text-left leading-none uppercase select-none z-20">
                <span>Estructura de Coordinacion Academica</span>
                <span className="text-cyan-400 font-bold mt-1">Nodos Activos: Red de Transicion Cientifica</span>
              </div>
            </motion.div>
          </div>
        </div>
