...
interface ProvinceDetail {
  ...
  gestion_estatal?: { alcalde?: string; regidores?: string[]; periodo?: string };
  servicios_publicos?: { nombre: string; tipo: string; estado: string; ubicacion?: string }[];
}
...
const emptyDetail = (provinceName: string): ProvinceDetail => ({
  ...
  data_sources: [],
  lugares_turisticos: [],
  gestion_estatal: { alcalde: '', regidores: [], periodo: '' },
  servicios_publicos: [],
});
...
export const ProvinciasDetalleAdmin: React.FC = () => {
  ...
  const [existingIndicatorNames, setExistingIndicatorNames] = useState<string[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [detailRes, districtsRes, indicatorsRes, namesRes] = await Promise.all([
        ...
        supabase.from('province_indicators').select('nombre')
      ]);

      const d = detailRes.data || emptyDetail(provinceName);
      setDetail({ 
        ...emptyDetail(provinceName), 
        ...d,
        gestion_estatal: d.gestion_estatal || { alcalde: '', regidores: [], periodo: '' },
        servicios_publicos: d.servicios_publicos || []
      });
      ...
      if (namesRes.data) {
        const uniqueNames = Array.from(new Set(namesRes.data.map((x: any) => x.nombre).filter(Boolean)));
        setExistingIndicatorNames(uniqueNames as string[]);
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleSaveDetail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast({ message: `✅ Info de ${provinceName} guardada correctamente.`, type: 'success' });
      loadData();
    } catch (e: any) {
        ...
        <div className="space-y-4">
          ...
          {activeTab === 'info' && (
            <form onSubmit={handleSaveDetail} className="space-y-5">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                ...
                <div className="space-y-4 bg-zinc-950 border border-zinc-900 p-5">
                  ...
                  <div>
                    ...
                    <input className="w-full bg-black border border-zinc-800 text-white text-xs p-2 font-mono focus:border-cyan-500 outline-none"
                      value={detail.photo_url || ''} onChange={e => setDetail({...detail, photo_url: e.target.value})} placeholder="https://..." />
                    {detail.photo_url && (
                ...
                <div className="space-y-4 bg-zinc-950 border border-zinc-900 p-5">
                  ...
                  <div>
                    ...
                    <textarea rows={4} className="w-full bg-black border border-zinc-800 text-white text-xs p-2 font-mono focus:border-cyan-500 outline-none resize-none"
                      ...
                      placeholder="Descripción territorial completa..." />
                  </div>
              ...
              {/* State Management & Public Services sections */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-4">
                ...
                <div className="space-y-4 bg-zinc-950 border border-zinc-900 p-5">
                  <h3 className="text-xs font-bold text-white uppercase font-mono border-b border-zinc-900 pb-2 flex items-center gap-2">
                    ...
                    Gestión Estatal / Municipal
                  </h3>
                  <div className="space-y-3">
                    <div>
                      <label className="text-[9px] font-mono text-zinc-500 uppercase block mb-1">Alcalde Provincial</label>
                      <input className="w-full bg-black border border-zinc-800 text-white text-xs p-2 font-mono focus:border-cyan-500 outline-none"
                        value={detail.gestion_estatal?.alcalde || ''} 
                        onChange={e => setDetail({
                          ...detail, 
                          gestion_estatal: { ...detail.gestion_estatal, alcalde: e.target.value }
                        })} 
                        placeholder="Ej: Joaquín Ramírez Gamarra" />
                    </div>
                    <div>
                      <label className="text-[9px] font-mono text-zinc-500 uppercase block mb-1">Período de Gestión</label>
                      <input className="w-full bg-black border border-zinc-800 text-white text-xs p-2 font-mono focus:border-cyan-500 outline-none"
                        value={detail.gestion_estatal?.periodo || ''} 
                        onChange={e => setDetail({
                          ...detail, 
                          gestion_estatal: { ...detail.gestion_estatal, periodo: e.target.value }
                        })} 
                        placeholder="Ej: 2023 - 2026" />
                    </div>
                    <div>
                      <label className="text-[9px] font-mono text-zinc-500 uppercase block mb-1">Regidores (separados por coma)</label>
                      <input className="w-full bg-black border border-zinc-800 text-white text-xs p-2 font-mono focus:border-cyan-500 outline-none"
                        value={(detail.gestion_estatal?.regidores || []).join(', ')} 
                        onChange={e => setDetail({
                          ...detail, 
                          gestion_estatal: { ...detail.gestion_estatal, regidores: e.target.value.split(',').map(s => s.trim()).filter(Boolean) }
                        })} 
                        placeholder="Regidor 1, Regidor 2, Regidor 3" />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4 bg-zinc-950 border border-zinc-900 p-5">
                  <h3 className="text-xs font-bold text-white uppercase font-mono border-b border-zinc-900 pb-2 flex items-center gap-2">
                    ...
                    Servicios Públicos Registrados
                  </h3>
                  <div className="space-y-3 max-h-[220px] overflow-y-auto pr-1">
                    {(detail.servicios_publicos || []).map((serv, sIdx) => (
                      <div key={sIdx} className="bg-black border border-zinc-900 p-3 space-y-2 relative group">
                        <div className="flex items-center justify-between">
                          <span className="text-[9px] font-mono text-cyan-400 uppercase">Servicio #{sIdx + 1}</span>
                          <button type="button"
                            onClick={() => {
                              const updated = [...(detail.servicios_publicos || [])];
                              updated.splice(sIdx, 1);
                              setDetail({...detail, servicios_publicos: updated});
                            }}
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <input className="w-full bg-zinc-900 border border-zinc-800 text-white text-[11px] p-1.5 font-mono focus:border-cyan-500 outline-none"
                            placeholder="Nombre del servicio (ej: Posta Médica)" value={serv.nombre || ''}
                            onChange={e => {
                              const updated = [...(detail.servicios_publicos || [])];
                              updated[sIdx] = { ...updated[sIdx], nombre: e.target.value };
                              setDetail({...detail, servicios_publicos: updated});
                            }} />
                          <select className="w-full bg-zinc-900 border border-zinc-800 text-white text-[11px] p-1.5 font-mono focus:border-cyan-500 outline-none"
                            value={serv.tipo}
                            onChange={e => {
                              const updated = [...(detail.servicios_publicos || [])];
                              updated[sIdx] = { ...updated[sIdx], tipo: e.target.value };
                              setDetail({...detail, servicios_publicos: updated});
                            }} >
                          ...
                          <select className="w-full bg-zinc-900 border border-zinc-800 text-white text-[11px] p-1.5 font-mono focus:border-cyan-500 outline-none"
                            ...
                            onChange={e => {
                              const updated = [...(detail.servicios_publicos || [])];
                              updated[sIdx] = { ...updated[sIdx], estado: e.target.value };
                              setDetail({...detail, servicios_publicos: updated});
                            }} >
                          ...
                          <input className="w-full bg-zinc-900 border border-zinc-800 text-white text-[11px] p-1.5 font-mono focus:border-cyan-500 outline-none"
                            placeholder="Ubicación / Sector" value={serv.ubicacion || ''}
                            onChange={e => {
                              const updated = [...(detail.servicios_publicos || [])];
                              updated[sIdx] = { ...updated[sIdx], ubicacion: e.target.value };
                              setDetail({...detail, servicios_publicos: updated});
                            }} />
                        </div>
                      </div>
                    ))}
                    {(detail.servicios_publicos || []).length === 0 && (
                      <div className="py-6 text-center text-zinc-600 text-[10px] font-mono border border-dashed border-zinc-800">
                        No hay servicios públicos registrados.<br />
                        <span className="text-cyan-700">Registra un nuevo servicio a continuación.</span>
                      </div>
                    )}
                  </div>
                  <button type="button"
                    onClick={() => setDetail({
                      ...detail, 
                      servicios_publicos: [...(detail.servicios_publicos || []), { nombre: '', tipo: 'Salud', estado: 'Bueno', ubicacion: '' }]
                    })}
                    className="w-full flex items-center justify-center gap-1 px-3 py-2 bg-zinc-900 text-zinc-400 text-[10px] font-mono uppercase cursor-pointer hover:text-white border border-zinc-800 hover:border-zinc-700">
                    <Plus className="h-3.5 w-3.5" /> Agregar Ficha de Servicio
                  </button>
                </div>
              </div>
              ...
              <button type="submit" disabled={saving}
                ...
                Guardar Todo (Info, Gestión, Servicios)
              </button>
            </form>
          )}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    ...
                    <div className="col-span-2">
                      ...
                      <input 
                        ...
                        className="w-full bg-black border border-zinc-800 text-white text-xs p-2 font-mono focus:border-cyan-500 outline-none"
                        value={editingIndicator.nombre} 
                        onChange={e => setEditingIndicator({...editingIndicator, nombre: e.target.value})}
                        placeholder="Ej: Tasa de pobreza monetaria" 
                      />
                      <datalist id="indicator-names">
                        {existingIndicatorNames.map(name => (
                    ...
                    <div>
                      ...
                      <input className="w-full bg-black border border-zinc-800 text-white text-xs p-2 font-mono focus:border-cyan-500 outline-none"
                        value={editingIndicator.unidad || ''} onChange={e => setEditingIndicator({...editingIndicator, ...editingIndicator, unidad: e.target.value})}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                {indicators.filter(ind => indCatFilter === 'all' || ind.categoria === indCatFilter).length === 0 && (
                  <div className="col-span-3 py-12 text-center text-zinc-600 text-xs font-mono border border-dashed border-zinc-900">
                    No hay indicadores en esta categoría.<br />
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                {districts.filter(d => districtFilter === 'all' || d.tipo === districtFilter).length === 0 && (
                  <div className="col-span-3 py-12 text-center text-zinc-600 text-xs font-mono border border-dashed border-zinc-900">
                    No hay distritos o caseríos.<br />
                    <span className="text-cyan-700">Agrega uno con el botón de arriba.</span>
                  </div>
                )}
              </div>
